        ...
        for item in data:
            ...
            try:
                storage_context = StorageContext.from_defaults(
                    persist_dir=f"./sep_circulaire_prod_chimie_store/{circular_number}"
                )
                index = load_index_from_storage(storage_context)
            except:
                # Load data
                docs = SimpleDirectoryReader(
                    input_files=[f"./chemist_circ/{file_name}"]
                ).load_data()
                ...
                # Persist index
                index.storage_context.persist(persist_dir=f"./sep_circulaire_prod_chimie_store/{circular_number}")
            ...
            tool = QueryEngineTool(
                ...
                metadata=ToolMetadata(
                    object =f"Circulaire {circular_number}",
                    description=(
                        f"tool contient le circulaire qui est sur:{objet} sortie le {date}"
                    ),
                ),
            )
...
circulaire_query_engine_tools = create_tools_for_chapters('chemist_circ')